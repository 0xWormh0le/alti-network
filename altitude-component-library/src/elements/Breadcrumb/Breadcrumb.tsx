...
function buildBreadcrumbLastVisitedItems(locations: string[], state: any) {
    ...
    return linkableLocations
      ...
      .map((breadCrumbPath, index) => {
        ...
        return {
          ...
          pageName,
          render: () => (
            <Link to={nextLocation} className='Breadcrumb__item Breadcrumb__link'>
              {pageName}
            </Link>
          ),
          separatorEnd: true
...
export const Breadcrumb: React.FC<BreadcrumbProps> = ({ pageName, state, Icon }) => {
  ...
  return (
    <LocationHistoryContextConsumer>
      {({ breadcrumbLocations }) => {
        const itemsToRender = [...buildBreadcrumbLastVisitedItems(breadcrumbLocations, state), currentItem]
