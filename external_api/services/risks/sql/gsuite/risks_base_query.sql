...
SELECT DISTINCT
    ...
    risk.kw_count_data,
    df.file_name,
    df.owners_displayName,
    df.file_mimeType AS file_mimetype,
    df.webViewLink AS web_link,
    COALESCE(risk.owners_emailAddress, df.owners_emailAddress) AS owners_emailaddress,
    risk.risk_target,
    CASE WHEN risk.risk_type_id = 3010 THEN  -- 3010 currently is the only app-based risk type.
        (SELECT displayText FROM admin_directory_v1_tokens WHERE clientId LIKE concat(risk.risk_target, '%%') LIMIT 1)
...
LEFT JOIN drive_v3_filesmeta df USING (file_id)
ORDER BY {{order}} {{sort}}
